...
pub fn _assemble_kkt_matrix<T:FloatT>(
    ...
    let nnz_WtW_blocks = maps.WtWblocks.iter().fold(0,|acc,block| acc + block.len());
...
fn _kkt_assemble_fill<T:FloatT>(
        ...
        if cones.types[i] == SupportedCones::SecondOrderConeT {
            ...
            match shape{
                MatrixTriangle::Triu => {
                    ...
                    K.fill_colvec(&mut maps.SOC_u[socidx], headidx + n, col + 1, nvars); //v
                }
                MatrixTriangle::Tril => {
                    K.fill_rowvec(&mut maps.SOC_v[socidx], col    , headidx + n,nvars); //u
                    K.fill_rowvec(&mut maps.SOC_u[socidx], col + 1, headidx + n,nvars); //v
                }
            }
    ...
    match shape {
        ...
        MatrixTriangle::Tril => {
            ...
            maps.diag_full.copy_from_slice(&K.colptr[0..K.colptr.len()-1]);
            // and the diagonal of just the upper left
