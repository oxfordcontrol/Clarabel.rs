...
use crate::default::*;
use crate::default::kktsolvers::*;
use crate::kktsolvers::direct_quasidefinite::directquasidefinitekktsolver::DirectQuasidefiniteKKTSolver;
...
impl<T:FloatT> DefaultKKTSystem<T>
{
    pub fn new(
        ...
        cones: &ConeSet<T>,
        settings: &Settings<T>)->Self
    {

        let (m,n) = (data.m, data.n);
        let kktsolver = DirectQuasidefiniteKKTSolver::<T>::new(&data.P,&data.A,cones,m,n,settings);
        ...
        Self{kktsolver,x1,z1,x2,z2,workx,workz,work_conic}
    }
}
    ...
    T: FloatT,
{
    ...
    )
    {
        ...
        match steptype {
            ...
            cones.gemv_W(MatrixShape::T, tmp, Wtlinvds, T::one(), T::zero()); // Wᵀ(λ \ ds) = Wᵀ(tmp)
            }
            _ => {panic!("Bad step direction specified");}
        }
        ...
        let tau_num = rhs.τ - rhs.κ/variables.τ + data.q.dot(x1) + data.b.dot(z1) + two*data.P.symdot(ξ,x1);
        ...
        data: &DefaultProblemData<T>)
    {
        ...
        self.kktsolver.solve(Some(&mut variables.x), Some(&mut variables.s));
