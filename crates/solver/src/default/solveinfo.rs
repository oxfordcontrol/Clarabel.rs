...
macro_rules! expformat {
    ($fmt:expr,$val:expr) => {_exp_str_reformat(format!($fmt, $val))}
}
...
impl<T: FloatT> SolveInfo<T> for DefaultSolveInfo<T>
{
    ...
    fn print_header(
        ...
        cones: &ConeSet<T>)
        {
            ...
            println!("-------------------------------------------------------------");
            println!("           Clarabel.rs v{}  -  Clever Acronym              \n", VERSION);
    ...
    fn print_status(&self, settings: &Settings<T>){
        ...
        // print!("% .4e  ", info.cost_primal);
        // print!("% .4e  ", info.cost_dual);
        // print!("%.2e  ", info.res_primal);
        // print!("%.2e  ", info.res_dual);
        // print!("%.2e  ", info.ktratio);
        // print!("%.2e  ", info.μ);
        ...
        if self.iterations > 0  {
            //PJG: Don't know how to format these yet.
            //print!("%.2e  ", info.step_length);
    ...
    )
    {
        ...
        self.res_dual_inf   = T::max(residuals.Px.norm_scaled(dinv),
...
fn _print_conedims_by_type<T:FloatT>(cones: &ConeSet<T>, conetype: SupportedCones)
{
    ...
    if !cones.type_counts.contains_key(&conetype) {return}
...
fn _exp_str_reformat(mut thestr: String) -> String {
    ...
    let has_short_exp = {
        if !has_sign {thestr.len() == eidx + 2}
        else {thestr.len() == eidx + 3}
    };

    let chars;
    if !has_sign {
        if has_short_exp {chars = "+0";}
        else {chars = "+";}
    }
