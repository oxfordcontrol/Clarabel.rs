...
pub struct DefaultSettingsFFI<T: FloatT> {
    ...
    // warm start settings (note: not fully exposed in FFI interface)
...
where
    T: FloatT,
{
    fn from(settings: DefaultSettingsFFI<T>) -> Self {
        Self {
            ...
            warm_start_enable: settings.warm_start_enable,
            // FFI interface doesn't support Vec fields, so use defaults
            warm_start_x: None,
            warm_start_s: None,
            warm_start_z: None,
            warm_start_tau: None,
