    ...
    T: FloatT,
{
    ...
    fn unit_initialization(&self, z: &mut [T], s: &mut [T]) {
        ...
        s[..α.len()].iter_mut().enumerate().for_each(|(i, s)| *s = (T::one() + α[i]).sqrt());
    ...
    fn get_Hs(&self, Hsblock: &mut [T]) {
        ...
        let d1 = & self.d1;
        Hsblock[..self.dim1].iter_mut().enumerate().for_each(|(i,x)| *x = self.μ*d1[i]);
    ...
    fn mul_Hs(&mut self, y: &mut [T], x: &[T], _work: &mut [T]) {
        ...
        y.iter_mut().enumerate().for_each(|(i,x)| *x = coef_p*self.p[i]);
        y[..dim1].iter_mut().enumerate().for_each(|(i,x)| *x += d1[i]*x1[i] - coef_q*self.q[i]);
        y[dim1..].iter_mut().enumerate().skip(dim1).for_each(|(i,x)| *x += d2*x2[i] - coef_r*self.r[i]);
    ...
    fn combined_ds_shift(&mut self, shift: &mut [T], step_z: &mut [T], step_s: &mut [T], σμ: T) {
        ...
        let grad = & self.grad;
        shift.iter_mut().enumerate().for_each(|(i,x)| *x = σμ*grad[i]);
    }
    ...
    fn compute_barrier(&self, z: &[T], s: &[T], dz: &[T], ds: &[T], α: T) -> T {
        ...
        wq.iter_mut().enumerate().for_each(|(i,x)| *x = z[i] + α * dz[i]);
        barrier += self.barrier_dual(&wq);

        wq.iter_mut().enumerate().for_each(|(i,x)| *x = s[i] + α * ds[i]);
    ...
    T: FloatT,
{
    ...
    fn update_dual_grad_H(&mut self, z: &[T]) {
        ...
        let τ = &mut self.q;
        τ.iter_mut().enumerate().for_each(|(i,τ)| *τ = two*α[i]/z[i]);
        grad[..dim1].iter_mut().enumerate().for_each(|(i,grad)| *grad = -τ[i]*phi/ζ - (T::one()-α[i])/z[i]);
        grad.iter_mut().enumerate().skip(dim1).for_each(|(i,x)| *x = two*z[i]/ζ);
        ...
        let d1 = &mut self.d1;
        d1.iter_mut().enumerate().for_each(|(i,d1)| *d1 = τ[i]*phi/(ζ*z[i]) + (T::one()-α[i])/(z[i]*z[i]));
        self.d2 = two/ζ;
        ...
        T: FloatT,
    {
        ...
        z[..dim1].iter().enumerate().for_each(|(i,x)| barrier -= (*x).logsafe()*(T::one()-α[i]));
        ...
        T: FloatT,
    {
        ...
        if norm_r > T::epsilon() {
            ...
            minus_g[..dim1].iter_mut().enumerate().for_each(|(i,x)| *x = -(T::one()+α[i])/s[i]);
        }
    ...
    T: FloatT,
{
    ...
    let x0 = -norm_r.recip() + (ψ*norm_r + ((phi/norm_r/norm_r + ψ*ψ -T::one())*phi).sqrt())/(phi - norm_r*norm_r);

    // function for f(x) = 0
    let f0 = {
        |x: T| -> T {
            let mut fval = -(two*x/norm_r + x*x).logsafe();
            α.iter().enumerate().for_each(|(i,& αi)| fval += two*αi*((x*norm_r + (T::one() + αi)/αi).logsafe() - p[i].logsafe()));
    ...
    let f1 = {
        |x: T| -> T {
            ...
            α.iter().for_each(|& αi| fval += two*(αi)*norm_r/(norm_r*x + (T::one()+αi)/αi));
